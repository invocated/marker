...
def document_with_heading(html=None, empty=False, ignored=False):
    page = PageGroup(
        page_id=0,
        polygon=PolygonBox.from_bbox([0, 0, 600, 800]),
        children=[],
        structure=[],
        lowres_image=Image.new("RGB", (600, 800)),
    )
...
def assert_rendered_heading(document, heading, level):
    ...
    for content in (
        json.children[0].children[0].html,
        html.html,
        chunks.blocks[0].html,
    ):
...
def test_optional_rewrite_synchronizes_final_level(processor_cls, correction_type):
    ...
    service = Mock(
        return_value={
            ...
            "blocks": [{"id": str(heading.id), "html": "<h4>Introduction</h4>"}],
        }
    )
...
@pytest.mark.parametrize(
    "kwargs", [{"empty": True}, {"html": "<h3>Introduction</h3>", "ignored": True}]
)
...
def test_toc_identity_and_order():
    ...
    second = document.pages[0].add_block(
        SectionHeader, PolygonBox.from_bbox([10, 50, 200, 70])
    )
    ...
    assert document.table_of_contents == [
        {
            ...
            "heading_level": 3,
            "page_id": 0,
            "polygon": first.polygon.polygon,
        },
        {
            "title": "Second",
            "heading_level": 1,
            "page_id": 0,
            "polygon": second.polygon.polygon,
        },
    ]
...
def test_initialized_default_and_custom_processor_order(monkeypatch):
    ...
    monkeypatch.setattr(
        BaseConverter, "resolve_dependencies", lambda self, cls: cls.__new__(cls)
    )
    ...
    assert types.index(LLMSimpleBlockMetaProcessor) < types.index(
    ...
    assert [type(p) for p in converter.processor_list] == [
...
def test_hierarchy_inherits_and_resets_across_pages():
    ...
    expected_toc = [
        ...
        for title, level, heading in [
            ("Introduction", 1, first),
            ("Detail", 3, second),
            ("Next", 2, third),
        ]
    ]
    expected_hierarchies = [
        {1: str(first.id)},
        {1: str(first.id), 3: str(second.id)},
    ...
    assert [
        ...
        for page_output in json.children
        for block in page_output.children
    ] == expected_hierarchies
    assert [
        block.section_hierarchy for block in ChunkRenderer()(document).blocks
    ] == expected_hierarchies


@pytest.mark.parametrize(
    "original_type,response_type,html",
    [
...
def test_page_correction_does_not_relabel_blocks(original_type, response_type, html):
    ...
    assert len(document.table_of_contents) == (
        1 if original_type == "SectionHeader" else 0
    )
