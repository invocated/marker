...
CASES = json.loads(
    Path(__file__).with_name("table_preservation_cases.json").read_text()
)
...
def test_overlapping_disjoint_fragments_do_not_become_false_rows():
    ...
    for index, y in enumerate((20, 40, 60), 1):
        source += [
            ([(str(index), 0, 10), (str(index + 10), 60, 75)], y + 2, y + 10),
...
def test_overlapping_true_rows_keep_separate_identifiers():
    ...
    source += [
        (
            [(str(i), 0, 10), (str(i + 10), 50, 60), ("Effect" + str(i), 100, 140)],
            y,
            y + 12,
        )
    ...
    assert result is not None
    assert cells(result[0])[1:] == [
        [str(i), str(i + 10), "Effect" + str(i)] for i in range(1, 4)
    ]
...
def test_failed_table_keeps_overlapping_source_text(disable_ocr, mode):
    ...
    model = Mock(
        return_value=[SimpleNamespace(blocks=[SimpleNamespace(html="", error=True)])]
    )
...
def test_fallback_missing_clean_reference_is_not_accepted_as_complete():
    ...
    TableProcessor(model, {"min_recon_score": 1.1, "collect_table_diagnostics": True})(
        doc
    )
...
def test_staggered_three_column_rows_and_multicolumn_continuations():
    source = [
        ([("ID", 0, 10), ("Name", 30, 70), ("Description", 90, 150)], 0, 8),
...
def test_blank_first_cell_remains_a_separate_row():
    source = [
        ([("ID", 0, 10), ("Cost", 50, 60), ("Effect", 100, 150)], 0, 8),
...
def test_sparse_then_full_header_line_is_one_header():
    ...
    source += [
        (
            [(str(i), 0, 10), ("Name" + str(i), 30, 70), ("Effect" + str(i), 90, 200)],
            y,
            y + 8,
        )
...
def test_cleanup_consumes_one_represented_occurrence():
    ...
    first.polygon = PolygonBox.from_bbox([10, 10, 200, 25])
    first.html = "<p>same text</p>"
    page.block_id = max(b.block_id for b in page.children)
    second = page.add_block(Text, PolygonBox.from_bbox([10, 30, 200, 45]))
    second.html = "<p>same text</p>"
...
def test_processor_rejects_raw_span_crossing_detected_boundary():
    ...
    doc = document(source)
    TableProcessor(Mock(), {"disable_ocr": True, "collect_table_diagnostics": True})(
        doc
    )
    trace = doc.table_diagnostics[0]
...
def test_header_can_be_wider_than_numeric_column_without_crossing_neighbor():
    ...
    source += [
        (
            [(str(i), 0, 10), ("Name" + str(i), 30, 70), ("Effect" + str(i), 90, 180)],
            y,
            y + 8,
        )
...
def test_named_dash_column_is_a_real_column():
    ...
    result = reconstruct_table_html(
        table_lines_from_pdftext(page_data(source), [0, 0, 450, 100])
    )
    assert result is not None
    assert cells(result[0]) == [
        ["ID", "Missing", "Value"],
...
def test_underscore_placeholder_policy_is_explicit(preserve):
    ...
    selected = table_lines_from_pdftext(
        page_data(source), [0, 0, 450, 100], preserve_placeholders=preserve
    )
    assert (
        any(text == "_____" for spans, _, _ in selected for text, _, _ in spans)
        == preserve
    )
...
@pytest.mark.parametrize(
    "renderer_cls", [HTMLRenderer, MarkdownRenderer, JSONRenderer, ChunkRenderer]
)
def test_converter_table_path_uses_real_processor_and_renderer(
    monkeypatch, converter_name, renderer_cls
):
    from importlib import import_module

    module = import_module("marker.converters." + converter_name)
    cls = getattr(
        module, "PdfConverter" if converter_name == "pdf" else "TableConverter"
    )
    doc = document(lines())
    ...
    converter.resolve_dependencies = (
        lambda dependency: renderer_cls() if dependency is renderer_cls else Mock()
    )
    monkeypatch.setattr(
        module, "provider_from_filepath", lambda _: lambda *args: object()
    )
    monkeypatch.setattr(
        module, "DocumentBuilder", lambda config: Mock(return_value=doc)
    )
    ...
    assert cells(doc.pages[0].children[0].html) == [
        ["ID", "Cost", "Effect"],
        ["1", "11", "Same"],
        ["2", "12", "Same"],
        ["3", "13", "Same"],
...
@pytest.mark.parametrize(
    "block_name,preserve",
    [("Table", True), ("Form", False), ("TableOfContents", False)],
)
def test_processor_placeholder_policy_matches_block_type(
    monkeypatch, block_name, preserve
):
    ...
    replacement = getattr(blocks, block_name)(
        polygon=old.polygon, page_id=0, block_id=0
    )
...
def test_form_cleanup_preserves_unrelated_text():
    ...
    page = doc.pages[0]
    form = Form(
        polygon=page.children[0].polygon,
        page_id=0,
        block_id=0,
        html="<form><label>Name</label><input/></form>",
    )
    page.children[0] = form
    page.structure[0] = form.id
    child = page.children[1]
    child.polygon = PolygonBox.from_bbox([10, 10, 100, 20])
    child.html = "<p>Name</p>"
    page.block_id = 1
    unrelated = page.add_block(Text, PolygonBox.from_bbox([10, 30, 100, 40]))
    unrelated.html = "<p>Unrelated</p>"
    page.add_structure(unrelated)
    TableProcessor(Mock())(doc)
...
def test_ocr_reference_comparison_preserves_case_uncertainty(ocr_word, missing):
    html = (
        "<table><tr><th>ID</th><th>Cost</th><th>Effect</th></tr>"
        + "".join(
            "<tr><td>"
            + str(i)
            + "</td><td>"
    ...
    model = Mock(
        return_value=[SimpleNamespace(blocks=[SimpleNamespace(html=html, error=False)])]
    )
    doc = document(lines())
    TableProcessor(model, {"min_recon_score": 1.1, "collect_table_diagnostics": True})(
        doc
...
def test_overlapping_column_spans_require_review():
    ...
    source += [
        (
            [(str(i), 0, 80), (str(i + 10), 50, 90), ("Effect" + str(i), 100, 140)],
            y,
            y + 8,
        )
        for i, y in [(1, 20), (2, 40), (3, 60)]
