...
VENDORED_RULES_LF_HASH = (
    "eb069771c2e6f5b3820ca82b0879ecb5fbb403312b08eac9d0bbbce72a5351da"
)
EXPECTED = [
    ["Item", "Count", "Note"],
    ["Alpha", "12", "Blue"],
...
def check_endpoint(endpoint):
    if endpoint:
        ...
        if (
            ...
            raise ValueError(
                "Use a loopback inference endpoint without embedded credentials or query"
            )
...
def main():
    ...
    parser.add_argument(
        "--case", choices=["table", "multi", "synthetic"], required=True
    )
    ...
    if args.case == "table":
        ...
        rules = [
            json.loads(x)
            for x in args.rules.read_text().splitlines()
        ...
        source = checked(
            repo / "tests/data/olmocr_bench/pdfs/multi_column_page1.pdf", MULTI_HASH
        )
        rule_path = repo / "tests/data/olmocr_bench/tests.jsonl"
        if digest(rule_path) not in {VENDORED_RULES_HASH, VENDORED_RULES_LF_HASH}:
            raise ValueError(
                "Vendored rules do not match the reviewed LF or CRLF bytes"
            )
        rules = [
            json.loads(x)
            for x in rule_path.read_text().splitlines()
            if json.loads(x)["pdf"] == "multi_column_page1.pdf"
        ]
    else:
        source = (
            checked(args.source, args.source_sha256) if args.source else synthetic(out)
        )
    source = source.resolve()
    source_hash = digest(source)
    code = {
        str(p.relative_to(repo)): digest(p)
        for p in sorted((repo / "marker").rglob("*.py"))
    }
    config = dict(
        mode=args.mode,
        page_range=[0],
        ...
        collect_table_diagnostics=args.diagnostics,
    )
    record = dict(
        case=args.case,
    ...
    if not endpoint:
        ...
        if "--revision" in extra:
            ...
            os.environ["VLLM_EXTRA_ARGS"] = " ".join(
                extra + ["--revision", args.model_revision]
            )
        record["model_revision_verification"] = (
            "revision passed to owned vLLM launch; retain startup log"
        )
    ...
    if (
        not Path(sys.modules["marker.converters.pdf"].__file__)
        .resolve()
        .is_relative_to(repo)
    ):
    ...
    record["packages"] = {
        ...
        for p in ["marker-pdf", "surya-ocr", "torch", "pytest", "Pillow", "psutil"]
    }
        ...
        while not stop.wait(0.1):
            ...
            for p in [process, *process.children(recursive=True)]:
                try:
                    ...
                    pass
            peaks["process_tree_rss_sum_bytes"] = max(
                peaks["process_tree_rss_sum_bytes"], total
            )
    ...
    def table_call(processor, document):
        ...
        for b in document.contained_blocks(processor.block_types):
            path = (
                "existing_html"
                if before.get(str(b.id))
                else b.text_extraction_method or "unresolved"
            )
            snapshots.append(
                dict(
                    id=str(b.id),
    ...
    try:
        ...
        if surya_settings.SURYA_INFERENCE_URL:
            record["model_revision_verification"] = (
                "existing loopback endpoint; caller must retain server revision evidence"
            )
        models = create_model_dict()
        record["diagnostics_supported"] = hasattr(
            TableProcessor, "collect_table_diagnostics"
        )
        record["effective_inference_settings"] = {
            k: getattr(surya_settings, k, None)
            for k in (
                "SURYA_MODEL_CHECKPOINT",
                "VLLM_DOCKER_IMAGE",
                "VLLM_GPU_MEMORY_UTILIZATION",
                ...
                "VLLM_MTP_TOKENS",
                "VLLM_GPU_TYPE",
                "SURYA_INFERENCE_STARTUP_TIMEOUT",
        ...
        while extra:
            ...
            if key == "--enforce-eager":
                ...
                raise ValueError(
                    "Unrecorded extra inference argument; extend the reviewed allowlist first"
                )
        record["effective_inference_settings"].update(
            max_num_batched_tokens=int(
                recorded_args.get("--max-num-batched-tokens", batched_tokens)
            ),
        ...
        with (
            patch.object(TableProcessor, "__call__", table_call),
            patch.object(recognition_type, "__call__", recognition_call),
        ):
        ...
        for name, image in images.items():
            ...
            if not destination.is_relative_to(out) or destination.exists():
                raise ValueError(
                    "Image reference escapes or overwrites the result directory"
                )
        ...
        write_json(
            out / "tables.json",
            dict(
                after_table_processor=snapshots,
                final=final,
        ...
        if args.case == "multi":
            checker = runpy.run_path(
                str(repo / "tests/converters/test_olmocr_bench.py")
            )["_check"]
            checks = [dict(id=r["id"], result=checker(r, text)) for r in rules]
            record["vendored_checker_sha256"] = digest(
                repo / "tests/converters/test_olmocr_bench.py"
            )
        else:
            checks = [
                dict(id=r["id"], result=load_single_test(r).run(text)) for r in rules
            ]
        if args.case == "table":
            tables = parse_markdown_tables(text) + parse_html_tables(text)
            shapes = [
                [
                    max((r for r, c in t.cell_text), default=-1) + 1,
                    max((c for r, c in t.cell_text), default=-1) + 1,
                ]
                for t in tables
            ]
            ...
            header = (
                [tables[0].cell_text.get((0, c), "").strip() for c in range(7)]
                if tables
                else []
            )
            record["supplementary_checks"] = [
                dict(
                    id="public_table_shapes",
                    result=[shapes == [[5, 7], [6, 5]], f"Observed {shapes}"],
                ),
                dict(
                    id="public_first_table_header",
                    result=[header == expected_header, f"Observed {header}"],
                ),
            ]
        if args.case == "synthetic":
            ...
            exact = (
                ...
                and {
                    ...
                    for position, value in tables[0].cell_text.items()
                }
                == expected
            )
            checks = [
                dict(
                    id="synthetic_exact_cells",
                    result=[
                        exact,
                        "Exactly one table and the exact expected cell map; no extra cells",
                    ],
                )
            ]
        record.update(
            status="converted",
            checks=checks,
            output_sha256=digest(out / "output.md"),
            effective_extract_images=converter.resolve_dependencies(
                converter.renderer
            ).extract_images,
        )
        ...
        record.update(
            memory=peaks,
            memory_scope="100ms sampled parent/process-tree RSS sum; shared pages may double-count; excludes Docker/GPU/external servers",
    ...
    return (
        ...
        and all(
            x["result"][0]
            for x in record["checks"] + record.get("supplementary_checks", [])
        )
        else 1
    )
